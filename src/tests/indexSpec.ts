...
describe('endpoint: /api/images', (): void => {
  ...
  it('Test The Image: Name,width and height', async (): Promise<void> => {
    const response: supertest.Response = await request.get(
      '/api/images?imgName=crow&width=200&height=200'
    );
  ...
  //-Width
  it('Test /api/images?imgName=crow&width=-200&height=200 with Wrong args', async (): Promise<void> => {
    const response: supertest.Response = await request.get(
      '/api/images?imgName=crow&width=-200&height=200'
    );
